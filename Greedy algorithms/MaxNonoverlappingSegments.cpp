...
whose positions are given in arrays A and B. For each I (0 ≤ I < N) the position of segment I is from A[I] to B[I] (inclusive).
...
int solution(std::vector<int>& A, std::vector<int>& B) {
    unsigned int numberOfNOSegments =  A.size();
    if (numberOfNOSegments == 0)
    {
        return 0;
    }
    unsigned int begin = A.back();
    unsigned int end = B.back();
